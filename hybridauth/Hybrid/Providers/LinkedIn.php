	...
	*/
	function getUserProfile()
	{
		try{
			...
			$response = $this->api->profile('~:(id,first-name,last-name,public-profile-url,picture-url,date-of-birth,phone-numbers,connections,summary,positions:(id,title,summary,start-date,end-date,is-current,company:(id,name,type,size,industry,ticker)),educations:(id,school-name,field-of-study,start-date,end-date,degree,activities,notes),skills:(id,skill:(name)))');
			
			
			/*https://api.linkedin.com/v1/people-search:
			//(people:(id,first-name,last-name,headline,picture-url,industry,positions:(id,title,summary,start-date,end-date,is-current,company:(id,name,type,size,industry,ticker)),educations:(id,school-name,field-of-study,start-date,end-date,degree,activities,notes)),num-results)?first-name=parameter&last-name=parameter
			*/
		}
		...
		if( isset( $response['success'] ) && $response['success'] === TRUE ){
			...
			print_r($degrees);
			print "</pre>";exit;
			$skillAll=substr($skill,1);
			echo "my degrees ".$skillAll;exit;*/
			...
			$this->user->profile->jobtitle       = (string) $data->{'positions'}->{'position'}->{'title'};
			$this->user->profile->companyname       = (string) $data->{'positions'}->{'position'}->{'company'}->{'name'}; 
			/** getting education names starts here*****/
			$education=$data->educations->education; 
			$degrees="";
			foreach($education as $singEdu){
				$degrees .=",".$singEdu->degree;
			}
			$degreesAll=substr($degrees,1);
			/** getting education names ends here*****/
			
			/** getting skill names starts here*****/
			$skills=$data->skills->skill; 
			foreach($skills as $sinSkills){
				$allSkill[]=$sinSkills->skill;
			}
			$skill="";
			foreach($allSkill as $skillNames){
				$skill .=",".$skillNames->name;
			}
			$skillAll=substr($skill,1);
			/** getting skill names ends here*****/
			
			/** getting connections names starts here*****/
			$conn=$data->connections->person;
			$conns="";
			$i=1;
			foreach($conn as $connSing){
				
				$conns .=$i.".".(string) $connSing->{'first-name'}." ".(string) $connSing->{'last-name'}.",";
				$i++;
			}
			/** getting connections names ends here*****/
			
			$this->user->profile->degrees       = $degreesAll; 
			$this->user->profile->skills       = $skillAll; 
			$this->user->profile->connections       = $conns; 
