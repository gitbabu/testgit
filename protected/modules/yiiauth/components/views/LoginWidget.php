<script type="text/javascript">
 $(document).ready(function() {
  <?php if(isset($this->fbinvite)){if($this->fbinvite == 'fbinvite'){?> 
   ...
   <?php }}?>
	...
	foreach ($providers as $provider){
		echo CHtml::link('<img src="'.Yii::app()->baseUrl.'/'.$path.$provider.'.png" alt="'.$provider.'" title="'.$provider.'" id="'.$provider.'"/>',
	...
	foreach($openid_servers as $image=>$server){
		echo CHtml::link('<img src="'.Yii::app()->baseUrl.'/'.$path.$image.'.png" alt="'.$server.'" title="'.$server.'"/>',
	...
	<!--<a href="#" id="openid" class="extra_info" title="Diffrent OpenID server">
		<img src="<?php echo Yii::app()->baseUrl;?>/<?php echo $path;?>openid.png" alt="Diffrent OpenID provider"/> 
	</a>-->
	
	<!-- target div for jQuery to set dynamic instructions to the user -->
	<div id="extra_info_label"></div><?php /*commented by ramesh*/?>
